...
export const Game = (props) => {
  ...
  /**
   * Sends the game score to server, updates the score for this game
   */
  function sendGameScore() {
    let rawTime = document.querySelector('.status__time').textContent;
    let [mins, secs] = rawTime.split(':');

    let difficultyLevel = document.querySelector('.status__difficulty-select')
      .options.selectedIndex;
    let userScore = mins * 60 + secs;
    ...
    axios
      .post(
        ...
        {
          headers: {
            Authorization: props.token.accessToken,
          },
        },
      )
      .catch((err) => {
